...
import { Session, ExerciseSettings, TrainingSession, UserSettings, Exercise, ExerciseSession, ExerciseCoachState, GlobalCoachState } from './models';
  ...
  constructor() {
    ...
    this.version(2).stores({
      ...
      return tx.table('sessions').toCollection().modify(session => {
        session.exerciseNames = session.exercises?.map((e: ExerciseSession) => e.exerciseName.toLowerCase()) ?? [];
      });
    });
